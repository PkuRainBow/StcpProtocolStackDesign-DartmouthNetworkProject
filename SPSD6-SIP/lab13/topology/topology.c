...
unsigned int topology_getCost(int fromNodeID, int toNodeID)
{
  ...
  while(fscanf(fp, "%s", host[i++]) > 0){
		fscanf(fp, "%s", host[i++]);
		fscanf(fp, "%s", distance);
		t1 = topology_getNodeIDfromname(host[i-1]);
		t2 = topology_getNodeIDfromname(host[i-2]);
		if( (t1==fromNodeID && t2==toNodeID) || (t2==fromNodeID && t1==toNodeID) ){
			//fclose(fp);	
			return atoi(distance);
		}
	}
	//fclose(fp);	
